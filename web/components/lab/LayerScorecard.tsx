...
export function LayerScorecard({ scorecard }: { scorecard: SequenceScorecard | undefined }) {
  const turn = scorecard?.turns[0];
  const grades = turn ? [turn.routing, turn.response, turn.gmail_safety, turn.identity, turn.duplicate, turn.context] : [];
  return <section className="evidence-panel scorecard-panel" data-testid="evidence-layer" aria-labelledby={`${scorecard?.system ?? 'unknown'}-scorecard`}>
    <div className="evidence-heading"><h4 id={`${scorecard?.system ?? 'unknown'}-scorecard`}>Layer scorecard</h4><span>{scorecard ? scorecard.passed ? '✓ Sequence pass' : '× Sequence fail' : 'Unavailable'}</span></div>
    {turn ? <div className="table-scroll" tabIndex={0} aria-label={`${scorecard.system} scorecard scroll area`}><table aria-label={`${scorecard.system === 'baseline' ? 'Baseline' : 'Enhanced'} layer scorecard`}><thead><tr><th>Layer</th><th>Result</th><th>Expected evidence</th><th>Observed gaps or contradictions</th></tr></thead><tbody>
