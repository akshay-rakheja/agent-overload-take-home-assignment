...
export function UsageCostPanel({ result, modelId }: { result: SystemRunResult; modelId: string | null }) {
  ...
  const total = [...timings].reverse().find((timing) => timing.phase === 'total_run') ?? timings.at(-1);
  const elapsed = total?.elapsed_ns !== undefined ? `${displayScalar(total.elapsed_ns)} ns` : total?.elapsed_ms !== undefined ? `${displayScalar(total.elapsed_ms)} ms` : result.timings.reason;
  return <section className="evidence-panel" data-testid="evidence-layer" aria-labelledby={`${result.turn_id}-usage`}>
    ...
    <dl className="metric-grid usage-grid">
      <div><dt>Model</dt><dd>{modelId ?? 'Unavailable'}</dd></div><div><dt>Provider</dt><dd>{total?.actual_provider !== undefined ? displayScalar(total.actual_provider) : total?.provider !== undefined ? displayScalar(total.provider) : 'Unavailable'}</dd></div><div><dt>Elapsed</dt><dd>{elapsed}</dd></div>
      {total?.started_monotonic_ns !== undefined && <div><dt>Monotonic start</dt><dd>{displayScalar(total.started_monotonic_ns)}</dd></div>}{total?.finished_monotonic_ns !== undefined && <div><dt>Monotonic finish</dt><dd>{displayScalar(total.finished_monotonic_ns)}</dd></div>}
