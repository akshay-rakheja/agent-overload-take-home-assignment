...
export function RoutingDispatchPanel({ result }: { result: SystemRunResult }) {
  ...
  const accepted = isRecord(result.accepted_dispatch.value) ? result.accepted_dispatch.value : {};
  ...
  return <section className="evidence-panel" data-testid="evidence-layer" aria-labelledby={`${result.turn_id}-routing`}>
    ...
    <dl className="routing-list">
      <Field label="Routing decision" observation={result.decision} fields={['action', 'agent_id', 'confidence', 'reasons']} />
      ...
      <div className="routing-field"><dt>Identity outcome</dt><dd><span>{accepted.new_agent_created === true ? 'Created' : accepted.new_agent_created === false && selected.agent_id ? 'Reused' : availabilityLabel(result.selected_identity.availability)}</span>{selected.name && <span className="stacked">{displayScalar(selected.name)}</span>}</dd></div>
      ...
      <div className="routing-field"><dt>Duplicates</dt><dd>{duplicates?.length === 0 ? 'No duplicates reported' : duplicates ? duplicates.map((item, index) => <span className="stacked" key={index}>{displayScalar(item)}</span>) : `${availabilityLabel(result.duplicates.availability)} · ${result.duplicates.reason ?? 'No evidence emitted'}`}</dd></div>
    </dl>
  </section>;
}
