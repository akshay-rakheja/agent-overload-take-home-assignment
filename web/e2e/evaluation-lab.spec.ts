...
test('keyboard flow runs a pair, preserves inferred baseline, expands candidates and controlled evidence', async ({ page }) => {
  ...
  await page.keyboard.press('Enter');
  await page.getByRole('button', { name: 'Expand candidate Atlas' }).focus();
  await page.keyboard.press('Enter');
  await expect(page.getByText('semantic 0.72')).toBeVisible();
...
test('active progress and one-side failure retain aligned successful evidence', async ({ page }) => {
  ...
  await expect(page.getByRole('region', { name: 'Baseline evidence' })).toContainText('Atlas found the controlled fixture');
...
test('exploratory evidence stays hidden while long-history facts remain visible', async ({ page }) => {
  ...
  for (const value of ['984,220 bytes', '10,000 entries', '9,968 omitted', 'Raw history preserved', 'No contamination detected']) await expect(page.getByText(value)).toBeVisible();
});
...
for (const viewport of [{ width: 1280, height: 800 }, { width: 1440, height: 900 }, { width: 1024, height: 800 }]) {
  for (const scale of [100, 1000]) test(`keeps ${scale.toLocaleString()}-agent evidence usable without page overflow at ${viewport.width}x${viewport.height}`, async ({ page }) => {
    ...
    const enhanced = large.pairs[0].outcomes[1].results[0];
    enhanced.roster_count.value = scale;
    const selectedIndex = scale === 100 ? 77 : 777;
    enhanced.prompt_exposure.value.identities = Array.from({ length: scale }, (_, index) => ({
      agent_id: `identity-${String(index).padStart(4, '0')}`, name: index === selectedIndex ? `Selected ${scale}-agent identity` : `Agent ${index + 1}`,
      purpose: `Purpose ${index + 1}`, status: 'cold', use_count: index,
    }));
    enhanced.selected_identity.value = { agent_id: `identity-${String(selectedIndex).padStart(4, '0')}`, name: `Selected ${scale}-agent identity` };
    await mockLab(page, { run: large });
    await page.goto('/lab');
    await page.getByRole('button', { name: 'Run scenario' }).click();
    await expect(page.getByText(`Roster size: ${scale.toLocaleString()}`)).toBeVisible();
    await expect(page.getByText(`Selected: Selected ${scale}-agent identity`)).toBeVisible();
...
test('browser fixture is accepted unchanged by the page contract', async ({ page }) => {
  ...
  expect(candidateNames).toEqual(['Atlas', 'Cedar', 'Delta']);
  await expect(page.getByText('controlled-query-sha256')).toBeVisible();
